...
ggplot(taxi_clean, aes(x = tip_amount, fill = PU_Borough)) +
  ...
  theme_minimal() +
  facet_wrap(~ PU_Borough, scales = "free_y")
...
ggplot(taxi_clean, aes(x = tip_amount, fill = DO_Borough)) +
  ...
  theme_minimal() +
  facet_wrap(~ DO_Borough, scales = "free_y")
```
...
* from the graphs we find that across all boroughs for pickup and drop off that tipping was not done seeing as the peaks in all the graphs are up around zero.  
